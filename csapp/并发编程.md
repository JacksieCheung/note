...
* 线程的两个状态： 可结合 和 分离
  ...
  在线程中，不像进程那样有父子关系。没开启一个线程，都会加入线程池。而主线程和其他线程区别就是主线程永远是线程池中的第一个线程，也是进程中第一个运行的。而我们开启的线程，称为对等线程~~（其实我一般都会说子线程，比较好理解）~~。
  ...
  默认情况下，为了避免内存泄漏，任何可结合线程都需要被其他线程显示回收。而分离的线程却不用，或者说不能被其他线程回收。分离的线程已经不归其他线程管了，而是由系统操作。实际些 web 服务器时，我们都会用分离线程。因为可结合线程的主线程需要等待它结束再回收，会造成阻塞，很明显我们不愿意等待。
